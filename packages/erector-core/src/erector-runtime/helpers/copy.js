...
import { ACTION_RUN, __CONFIG__ } from '../../constants';
...
export default function copy(source, destination, opt = {}) {
  ...
  return function* (state, store) {
    ...
    const isSourceDir = yield isDirectory(resolvedSource);
    yield createDir(isSourceDir ? resolvedDestination : path.dirname(resolvedDestination));
    yield new Promise(function(resolve, reject) {
      const transform = typeof opt.transform === 'function' ? decorateTransform(store, opt.transform, encoding, reject) : defaultTransform;
