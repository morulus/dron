      ...
      if (e.code=='MODULE_NOT_FOUND') {
        try {
          ...
          if (e.code=='MODULE_NOT_FOUND'||~e.message.indexOf('Cannot find global module')) {
            ...
            return new Error('Autoinstalling is not supported yet');
          } else {
            let shortPackageName = (packageName.split('/').pop());
            return new Error('Package '+shortPackageName+' has an errors. Run `dron debug '+packageName+'` to find a problem.');
          }
        }
      } else {
        console.log(e.stack);
