...
const ACTION_ASSIGN_REDUCER = require('dron-constants').ACTION_ASSIGN_REDUCER;
const ACTION_RUN = require('dron-constants').ACTION_RUN;
const LAST_REDUCER = require('dron-constants').LAST_REDUCER;
const ENV = require('dron-constants').ENV;
const run = require('./loopback-run.js');
const MESSAGE = require('./loopback-run.js').MESSAGE;
...
module.exports = function dronRun(fn, props, preloadedState, reducers) {
  let store;
  /**
   * Create middleware
   */
  let middleware = function(getState) {
    return function(next) {
      return function(action) {
        switch(action.type) {
          case ACTION_RUN:
            action.next(run(action.subject, action.state, store));
          break;
          case MESSAGE:
